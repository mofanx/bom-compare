"use strict";
(() => {
  // iframe/src/core/column-mapper.ts
  var COLUMN_ALIASES = {
  ...
  // iframe/src/core/parser/csv-parser.ts
  var SEPARATORS = ["	", ",", ";", "|"];
  ...
  function createBomRow(fields, mappings, rowIndex) {
    const row = {
      rowIndex,
      designator: "",
      footprint: "",
  ...
  function parse_BrtHLink(data, length) {
    ...
    if (tooltip2) o.Tooltip = tooltip2;
    return o;
  }
  ...
  function cell_set_hyperlink(cell, target, tooltip2) {
    if (!target) {
      ...
      if (tooltip2) cell.l.Tooltip = tooltip2;
    }
    return cell;
  }
  ...
  // iframe/src/core/parser/excel-parser.ts
  function parseExcel(data, fileName, sheetName) {
    ...
    const headerLineIndex = findHeaderLine2(lines);
    ...
    for (let i = headerLineIndex + 1; i < lines.length; i++) {
      ...
      const row = createBomRow2(fields, columnMappings, i);
      if (row.designator) {
        rows.push(row);
      }
    }
  ...
  function findHeaderLine2(lines) {
    const keywords = ["designator", "\u4F4D\u53F7", "ref", "reference"];
    for (let i = 0; i < Math.min(5, lines.length); i++) {
  ...
  function createBomRow2(fields, mappings, rowIndex) {
    const row = {
      rowIndex,
      designator: "",
      footprint: "",
  ...
  function detectEncoding(buffer) {
    ...
    while (i < Math.min(bytes.length, 4096)) {
      ...
      if (b > 127) {
        hasHighBytes = true;
        if ((b & 224) === 192) {
          if (i + 1 >= bytes.length || (bytes[i + 1] & 192) !== 128) {
            isValidUTF8 = false;
            break;
          }
          i += 2;
  ...
  var en_default = {
    ...
    selectSheetHint: "Select sheet to import",
    close: "Close",
    // Table headers
  ...
  function initI18n() {
    const saved = localStorage.getItem("bom-compare-language");
  ...
  function addI18nSupport() {
    initI18n();
    updateAllText();
  }

  // iframe/src/ui/sheet-selector.ts
  var currentResolve = null;
  function showSheetSelector(sheetNames, defaultSheet = "") {
    return new Promise((resolve) => {
      currentResolve = resolve;
      const overlay2 = document.createElement("div");
      overlay2.className = "dialog-overlay";
  ...
  async function selectSheetForExcel(data, fileName) {
  ...
  // iframe/src/core/parser/index.ts
  async function parseFile(file) {
    const ext = file.name.split(".").pop()?.toLowerCase() || "";
    if (ext === "xls" || ext === "xlsx") {
      const buffer2 = await file.arrayBuffer();
      const sheetName = await selectSheetForExcel(buffer2, file.name);
      return parseExcel(buffer2, file.name, sheetName);
    }
  ...
  function updateLoadingUI() {
    ...
    if (progressBarEl) progressBarEl.style.width = `${state.loadingProgress}%`;
  ...
  // iframe/src/ui/dialog.ts
  var overlay = null;
  function showRowDetailDialog(rowDiff) {
    closeDialog();
    ...
    header.innerHTML = `
		...
		<button class="dialog-close" title="\u5173\u95ED">\xD7</button>
	`;
    dialog.appendChild(header);
    const body = document.createElement("div");
    body.className = "dialog-body";
    ...
    const footerCloseBtn = footer.querySelector(".dialog-close");
  ...
  // iframe/src/ui/column-resize.ts
  ...
  function addTooltipSupport(element) {
    element.classList.add("has-tooltip");
  }

  // iframe/src/ui/editable.ts
  var editingCell = null;
  function initEditableTable(table) {
    ...
    if (save) {
      ...
      if (container?.id === "table-right" && state.diffResult) {
        ...
        if (rowDiff && rowDiff.newRow) {
          const th = td.closest("table")?.querySelector(`th[data-field]`);
          if (th) {
  ...
  function getFilteredRows() {
    ...
    if (state.sortField) {
      rows.sort((a, b) => {
        ...
        const valA = rowA?.[state.sortField] || "";
        const valB = rowB?.[state.sortField] || "";
  ...
  // iframe/src/ui/table.ts
  function renderTable(container, bomFile, side) {
    const table = document.createElement("table");
  ...
  function renderDiffTable(container, rows, side) {
    const table = document.createElement("table");
    table.className = "bom-table";
    const thead = document.createElement("thead");
    const headerRow = document.createElement("tr");
  ...
  function createDiffRow(row, rowDiff, side, index) {
    const tr = document.createElement("tr");
  ...
  function getRowClass(type, side) {
    switch (type) {
      case "changed":
        return "row-changed";
      case "added":
        return side === "new" ? "row-added" : "";
      case "removed":
        return side === "old" ? "row-removed" : "";
      default:
        return "row-same";
    }
  }
  function filterRows(rows) {
    ...
    if (state.sortField) {
      filtered = [...filtered].sort((a, b) => {
        ...
        const valA = String(rowA[state.sortField] || "").toLowerCase();
        const valB = String(rowB[state.sortField] || "").toLowerCase();
  ...
  function highlightLinkedRow(index, targetSide) {
  ...
  function showDetailDialog(rowDiff) {
    showRowDetailDialog(rowDiff);
  }
  function initRowHoverHighlight() {
  }

  // iframe/src/ui/drop-zone.ts
  var SUPPORTED_EXTENSIONS = [".csv", ".txt", ".xls", ".xlsx"];
  ...
  function setupDropZone(zoneId, side) {
    ...
    zone.addEventListener("drop", async (e) => {
      ...
      if (!isSupportedFile(file)) {
        ...
        setTimeout(() => zone.classList.remove("drag-reject"), 2e3);
  ...
  async function loadFile(file, side) {
    const pathInput = document.getElementById(`${side === "old" ? "old" : "new"}-file-path`);
  ...
  function initDropZones() {
    ...
    const fileInputOld = document.getElementById("file-input-old");
    const fileInputNew = document.getElementById("file-input-new");
    if (fileInputOld) {
  ...
  // iframe/src/core/comparator.ts
  var COMPARE_FIELDS = ["footprint", "quantity", "manufacturer", "lcscPart", "value", "description"];
  ...
  function buildDesignatorMap(rows) {
  ...
  function compareRows(oldRow, newRow) {
    const diffs = [];
  ...
  function sortRows(a, b) {
    const typeOrder = { same: 0, changed: 0, removed: 1, added: 2 };
  ...
  function exportReport(result) {
    ...
    utils.book_append_sheet(wb, summarySheet, "\u53D8\u66F4\u6458\u8981");
    ...
    utils.book_append_sheet(wb, detailSheet, "\u5DEE\u5F02\u660E\u7EC6");
  ...
  function navigateDiff(direction) {
    ...
    const diffIndices = state.diffResult.rows.map((row, i) => row.type !== "same" ? i : -1).filter((i) => i >= 0);
    ...
    if (direction === "next") {
      ...
      state.currentDiffIndex = next !== void 0 ? next : diffIndices[0];
    } else {
      const prev = [...diffIndices].reverse().find((i) => i < state.currentDiffIndex);
      state.currentDiffIndex = prev !== void 0 ? prev : diffIndices[diffIndices.length - 1];
    }
